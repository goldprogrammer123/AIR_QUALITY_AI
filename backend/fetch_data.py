...
df["aqi_no2"] = df["no2"].fillna(0).apply(lambda x: compute_aqi(x, no2_bp))
if "so2" in df.columns:
...
# Prepare ML Dataset
features = ["co2","humidity","pressure","temperature","hour","day","month"]
...
print(df[["pm25", "pm10", "co2"]].describe())

print("Rows:", len(df))

# Check existence of data bfore training
